---
layout: post
title: "Vue摄像头扫描文档到PDF"
date: 2024-12-17 10:15:53 +0800
categories: 文档扫描
tags: 
description: 文章讲述了如何构建一个Vue.js应用程序，使用摄像头扫描文档并将其保存为PDF文件。使用了Dynamsoft Mobile Web Capture SDK。
---

现代浏览器现在可以访问摄像头并运行图像处理来执行各种计算机视觉任务。在本文中，我们将基于Vue.js编写一个网页应用，用于从摄像头扫描文档图像并将其保存为PDF文件。
...
1. 具有边框检测和自动拍照功能的相机界面：
   ...
   ![perspective viewer](/album/2024/12/react-document-scanner/capture.jpg)
...
创建名为`dynamsoft.config.ts`的文件，文件内容如下，用于初始化许可证和资源。可以在此处申请[许可证](https://www.dynamsoft.com/customer/license/trialLicense/?product=dcv&package=cross-platform)。
...
接下来，在`src/components/Scanner.vue`路径创建一个文档扫描组件。
